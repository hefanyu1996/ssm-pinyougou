...
public class SeckillOrderController {
	...
	public Result submitOrder(Long id){
        ...
        if(!seckillOrderService.findUserByOrderExist(userId, id).isSuccess()){ // 查询用户是否重复下单

            return new Result(false,"亲，秒杀商品每种只限购一次哦");

        }

        try {
            seckillOrderService.submitOrder(id,userId);
            return new Result(true,"下单成功,请在1分钟内完成支付");
